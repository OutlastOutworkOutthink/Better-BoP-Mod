# Better Battle of Polytopia Mod — Alpha 0.5.7
...
Alpha 0.5.7 retains both the visible legacy setup row and UI2's later layout
...
at the top-left beside the back control. Press it to open a single-use Discord
OAuth page in the default browser. After approval, the game marks the profile
as connected, the PolyEconomic Bot registers the player, and the bot announces
the successful integration in the server's player-updates channel.

The link control is restored after all six known profile lifecycle paths:
start, enable, value refresh, screen update, account refresh, and language
refresh. The secure URL is also copied to the clipboard before browser handoff
...
Download the Alpha 0.5.7 release ZIP, extract it, and place the
...
```text
Better BoP Alpha 0.5.7 loaded: locked Oblivion plus Discord account linking.
```
