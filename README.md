# Better Battle of Polytopia Mod — Alpha 0.5.15
...
Alpha 0.5.15 retains both the visible legacy setup row and UI2's later layout
...
Alpha 0.5.15 retains the legacy tribe-info fix that replaces separate enabled and
...
and receives the Start button once both tribes are selected. Alpha 0.5.15 creates
...
The permanent Discord link is identity only. Alpha 0.5.15 separately proves its
...
Download the Alpha 0.5.15 release ZIP, extract it, and place the
...
```text
Better BoP Alpha 0.5.15 loaded: locked gameplay baselines plus resilient bot-provisioned Integrated Modded games.
```
