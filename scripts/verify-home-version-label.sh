...
grep -Fq 'BBoP Alpha 0.6.0' "$source_file"
...
if grep -Eq 'GetComponentsInChildren|Object\.Instantiate|HomeVersion(AfterLayout|OnShow|Layout|Refresh)Patch|\[HarmonyPatch\(typeof\(StartScreen_UI2\), "(Update|RunLayout|RefreshVersionInfo|OnShow|OnShowAfterLayout)"\)\]' "$source_file"; then
